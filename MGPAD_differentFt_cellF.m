function [ Z,Y,details] = MGPAD_differentFt_cellF(sys,Ptree,Tree,V,ops)
...
%monotonicity on primal with theta restart to [1 1]
...
while(j<ops.steps)
    ...
    if(j>1)
        if(mod(details.restart,2)==0)
            if(details.prm_cst(iter-1)>details.prm_cst(iter))
    ...
    details.dual_grad(iter)=0; % dual gradient
    for i=1:Nd-Ns
