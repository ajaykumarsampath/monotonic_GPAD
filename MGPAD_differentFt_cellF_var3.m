function [ Z,Y,details] = MGPAD_differentFt_cellF_var3(sys,Ptree,Tree,V,ops)
...
% Monotonicity in dual cost with restart theta=[1 1];
...
while(j<ops.steps)
    ...
    if(j>1)
        if(mod(details.restart,2)==0)
            if(details.dual_cst(iter-1)<details.dual_cst(iter))
